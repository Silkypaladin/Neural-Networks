...
class MLP:
    ...
    def go_backwards(self, y_train, out):
        ...
        w_updates_rev = self.prev_weights_updates[::-1]

        error = (out - y_train) * self.softmax(layer_outputs_rev[0], True)
        weight_updates = [np.outer(error, activated_outputs_rev[1])]
        bias_updates = [error]
        weights_modifiers = weights if self.prev_weights_updates[0] is None \
            else np.array(weights) - w_updates_rev * self.gamma

        for i in range(len(self.weights) - 1):
            error = np.dot(weights_modifiers[i].T, error) * MLP.activate(layer_outputs_rev[i + 1], functions_rev[i + 1], True)
