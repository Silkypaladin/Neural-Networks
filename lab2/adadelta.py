...
class MLP:
    ...
    def initialize_weights(self):
        for i in range(len(self.sizes) - 1):
            ...
            self.e_g = [w * 0 for w in self.weights]
            self.e_d = [w * 0 for w in self.weights]
    ...
    def update_parameters_batch(self, weight_updates, bias_updates, num_of_elems_in_batch):
        e = 0.0000001
        for i in range(len(self.weights)):
            self.e_g[i] = self.gamma * self.e_g[i] + (1 - self.gamma) * np.square(weight_updates[i])
            update = - np.sqrt((self.e_d[i] + e) / (self.e_g[i] + e)) * weight_updates[i]
            self.weights[i] += update / num_of_elems_in_batch

            self.e_d[i] = self.gamma + (1 - self.gamma) * np.square(update)
